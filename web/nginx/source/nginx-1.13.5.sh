...
cd nginx-1.13.5
./configure --prefix=/srv/nginx-1.13.5 \
...
rm -f /srv/nginx
ln -s /srv/nginx-1.13.5 /srv/nginx

strip /srv/nginx-1.13.5/sbin/nginx

cp /srv/nginx-1.13.5/conf/nginx.conf{,.original}

vim /srv/nginx-1.13.5/conf/nginx.conf <<VIM > /dev/null 2>&1
...
sed -i '/gzip  on;/a \
    include /etc/nginx/conf.d/*.conf;' /srv/nginx-1.13.5/conf/nginx.conf
