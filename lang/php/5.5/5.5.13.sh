...
cd /usr/local/src/
wget http://is1.php.net/distributions/php-5.5.13.tar.gz

if [ -s php-5.5.13.tar.gz ]; then
tar zxf php-5.5.13.tar.gz
cd php-5.5.13

./configure --prefix=/srv/php-5.5.13 \
--with-config-file-path=/srv/php-5.5.13/etc \
--with-config-file-scan-dir=/srv/php-5.5.13/etc/conf.d \
--enable-fpm \
--with-fpm-user=www \
--with-fpm-group=www \
--with-pear \
...
strip /srv/php-5.5.13/bin/php
strip /srv/php-5.5.13/bin/php-cgi
...
rm -f /srv/php
ln -s /srv/php-5.5.13/ /srv/php

vim /srv/php-5.5.13/etc/php-fpm.conf <<end > /dev/null 2>&1
:25,25s/;//
:32,32s/;//
...
vim /srv/php-5.5.13/etc/php.ini <<EOF > /dev/null 2>&1
...
#:706,706s!;include_path = ".:/php/includes"!include_path = ".:/srv/php-5.5.13/lib/php:/srv/php-5.5.13/share"!
#:728,728s!; extension_dir = "./"!extension_dir = "./:/srv/php-5.5.13/lib/php/extensions:/srv/php-5.5.13/lib/php/extensions/no-debug-non-zts-20121212"!
