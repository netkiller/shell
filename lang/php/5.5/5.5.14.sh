...
cd /usr/local/src/
wget http://is1.php.net/distributions/php-5.5.14.tar.gz

if [ -s php-5.5.14.tar.gz ]; then
tar zxf php-5.5.14.tar.gz
cd php-5.5.14

./configure --prefix=/srv/php-5.5.14 \
--with-config-file-path=/srv/php-5.5.14/etc \
--with-config-file-scan-dir=/srv/php-5.5.14/etc/conf.d \
--enable-fpm \
--with-fpm-user=www \
--with-fpm-group=www \
--with-pear \
...
strip /srv/php-5.5.14/bin/php
strip /srv/php-5.5.14/bin/php-cgi

mkdir -p /srv/php-5.5.14/etc/conf.d
cp php.ini-* /srv/php-5.5.14/etc/
cp /srv/php-5.5.14/etc/php.ini-production /srv/php-5.5.14/etc/php.ini
cp /srv/php-5.5.14/etc/php.ini-development /srv/php-5.5.14/etc/php.cli.ini
...
rm -f /srv/php
ln -s /srv/php-5.5.14/ /srv/php

vim /srv/php-5.5.14/etc/php-fpm.conf <<end > /dev/null 2>&1
:25,25s/;//
:32,32s/;//
...
vim /srv/php-5.5.14/etc/php.ini <<EOF > /dev/null 2>&1
:299,299s$;open_basedir =$open_basedir = /www/:/tmp/:/var/tmp/:/srv/php-5.5.14/lib/php/:/srv/php-5.5.14/bin/$
...
#:706,706s!;include_path = ".:/php/includes"!include_path = ".:/srv/php-5.5.14/lib/php:/srv/php-5.5.14/share"!
#:728,728s!; extension_dir = "./"!extension_dir = "./:/srv/php-5.5.14/lib/php/extensions:/srv/php-5.5.14/lib/php/extensions/no-debug-non-zts-20121212"!
...
vim /srv/php-5.5.14/etc/php.cli.ini <<EOF > /dev/null 2>&1
