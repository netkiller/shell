...
cd /usr/local/src/
wget http://php.net/distributions/php-7.0.7.tar.bz2

if [ -s php-7.0.7.tar.gz ]; then

tar jxf php-7.0.7.tar.bz2
cd php-7.0.7

./configure --prefix=/srv/php-7.0.7 \
--with-config-file-path=/srv/php-7.0.7/etc \
--with-config-file-scan-dir=/srv/php-7.0.7/etc/conf.d \
--mandir=/srv/php-7.0.7/man \
--enable-fpm \
--enable-opcache \
--with-fpm-systemd \
--with-fpm-user=www \
...
strip /srv/php-7.0.7/bin/php
strip /srv/php-7.0.7/bin/phpdbg
strip /srv/php-7.0.7/sbin/php-fpm 

mkdir -p /srv/php-7.0.7/etc/conf.d
mkdir -p /srv/php-7.0.7/etc/fpm.d
cp /srv/php-7.0.7/etc/pear.conf{,.original}
cp php.ini-* /srv/php-7.0.7/etc/
cp /srv/php-7.0.7/etc/php.ini-production /srv/php-7.0.7/etc/php.ini
cp /srv/php-7.0.7/etc/php.ini-production /srv/php-7.0.7/etc/php-cli.ini
cp /srv/php-7.0.7/etc/php-fpm.conf.default /srv/php-7.0.7/etc/php-fpm.conf
cp /srv/php-7.0.7/etc/php-fpm.d/www.conf.default /srv/php-7.0.7/etc/php-fpm.d/www.conf
...
vim /srv/php-7.0.7/etc/php-fpm.conf <<end > /dev/null 2>&1
:17,17s/;pid/pid/
...
vim /srv/php-7.0.7/etc/php-fpm.d/www.conf <<end > /dev/null 2>&1
...
vim /srv/php-7.0.7/etc/php.ini <<EOF > /dev/null 2>&1
:294,294s$;open_basedir =$open_basedir = /www/:/tmp/:/var/tmp/:/srv/php-7.0.7/lib/php/:/srv/php-7.0.7/bin/$
...
:389,389s/memory_limit = 128M/memory_limit = 32M/
:761,761s/;cgi.fix_pathinfo=1/cgi.fix_pathinfo=1/
...
#:706,706s!;include_path = ".:/php/includes"!include_path = ".:/srv/php-7.0.7/lib/php:/srv/php-7.0.7/share"!
#:728,728s!; extension_dir = "./"!extension_dir = "./:/srv/php-7.0.7/lib/php/extensions:/srv/php-7.0.7/lib/php/extensions/no-debug-non-zts-20121212"!
#:804,804s/upload_max_filesize = 2M/upload_max_filesize = 3M/

vim /srv/php-7.0.7/etc/php-cli.ini <<EOF > /dev/null 2>&1
