...
if [ -s php-5.6.16.tar.gz ]; then

tar zxf php-5.6.16.tar.gz
cd php-5.6.16

./configure --prefix=/srv/php-5.6.16 \
--with-config-file-path=/srv/php-5.6.16/etc \
--with-config-file-scan-dir=/srv/php-5.6.16/etc/conf.d \
--enable-fpm \
--enable-opcache \
--with-fpm-user=www \
--with-fpm-group=www \
...
strip /srv/php-5.6.16/bin/php
#strip /srv/php-5.6.16/bin/php-cgi
strip /srv/php-5.6.16/sbin/php-fpm 

mkdir -p /srv/php-5.6.16/etc/conf.d
mkdir -p /srv/php-5.6.16/etc/fpm.d
cp /srv/php-5.6.16/etc/pear.conf{,.original}
cp php.ini-* /srv/php-5.6.16/etc/
cp /srv/php-5.6.16/etc/php.ini-production /srv/php-5.6.16/etc/php.ini
cp /srv/php-5.6.16/etc/php.ini-production /srv/php-5.6.16/etc/php-cli.ini
cp /srv/php-5.6.16/etc/php-fpm.conf.default /srv/php-5.6.16/etc/php-fpm.conf

#cp /srv/php-5.6.16/etc/php.ini-development /srv/php-5.6.16/etc/php.ini
#cp /srv/php-5.6.16/etc/php.ini-development /srv/php-5.6.16/etc/php-cli.ini

cp ./sapi/fpm/php-fpm.service /etc/systemd/system/php-fpm.service 
sed -i 's:${prefix}:/srv/php-5.6.16:g' /etc/systemd/system/php-fpm.service
sed -i 's:${exec_prefix}:/srv/php-5.6.16:g' /etc/systemd/system/php-fpm.service

systemctl enable php-fpm

vim /srv/php-5.6.16/etc/php-fpm.conf <<end > /dev/null 2>&1
:25,25s/;//
:32,32s/;//
...
vim /srv/php-5.6.16/etc/php.ini <<EOF > /dev/null 2>&1
:298,298s$;open_basedir =$open_basedir = /www/:/tmp/:/var/tmp/:/srv/php-5.6.16/lib/php/:/srv/php-5.6.16/bin/$
...
#:706,706s!;include_path = ".:/php/includes"!include_path = ".:/srv/php-5.6.16/lib/php:/srv/php-5.6.16/share"!
#:728,728s!; extension_dir = "./"!extension_dir = "./:/srv/php-5.6.16/lib/php/extensions:/srv/php-5.6.16/lib/php/extensions/no-debug-non-zts-20121212"!
#:804,804s/upload_max_filesize = 2M/upload_max_filesize = 3M/

vim /srv/php-5.6.16/etc/php-cli.ini <<EOF > /dev/null 2>&1
