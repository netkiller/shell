...
dnf install -y systemd-devel libacl-devel
...
if [ -s php-5.6.10.tar.gz ]; then

tar zxf php-5.6.10.tar.gz
cd php-5.6.10

./configure --prefix=/srv/php-5.6.10 \
--with-config-file-path=/srv/php-5.6.10/etc \
--with-config-file-scan-dir=/srv/php-5.6.10/etc/conf.d \
--enable-fpm \
--enable-opcache \
--with-fpm-user=www \
--with-fpm-group=www \
...
strip /srv/php-5.6.10/bin/php
#strip /srv/php-5.6.10/bin/php-cgi
strip /srv/php-5.6.10/sbin/php-fpm 

mkdir -p /srv/php-5.6.10/etc/conf.d
mkdir -p /srv/php-5.6.10/etc/fpm.d
cp /srv/php-5.6.10/etc/pear.conf{,.original}
cp php.ini-* /srv/php-5.6.10/etc/
cp /srv/php-5.6.10/etc/php.ini-production /srv/php-5.6.10/etc/php.ini
cp /srv/php-5.6.10/etc/php.ini-development /srv/php-5.6.10/etc/php-cli.ini
cp /srv/php-5.6.10/etc/php-fpm.conf.default /srv/php-5.6.10/etc/php-fpm.conf
...
rm -f /srv/php
ln -s /srv/php-5.6.10/ /srv/php

vim /srv/php-5.6.10/etc/php-fpm.conf <<end > /dev/null 2>&1
:25,25s/;//
:32,32s/;//
...
vim /srv/php-5.6.10/etc/php.ini <<EOF > /dev/null 2>&1
:298,298s$;open_basedir =$open_basedir = /www/:/tmp/:/var/tmp/:/srv/php-5.6.10/lib/php/:/srv/php-5.6.10/bin/$
...
#:706,706s!;include_path = ".:/php/includes"!include_path = ".:/srv/php-5.6.10/lib/php:/srv/php-5.6.10/share"!
#:728,728s!; extension_dir = "./"!extension_dir = "./:/srv/php-5.6.10/lib/php/extensions:/srv/php-5.6.10/lib/php/extensions/no-debug-non-zts-20121212"!
#:804,804s/upload_max_filesize = 2M/upload_max_filesize = 3M/

vim /srv/php-5.6.10/etc/php.cli.ini <<EOF > /dev/null 2>&1
